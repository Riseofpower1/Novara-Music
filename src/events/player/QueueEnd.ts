...
export default class QueueEnd extends Event {
	...
	public async run(
		...
		// Log queue end event for dashboard analytics
		const analyticsService = new AnalyticsService();
		const currentUserId = player.get<string | undefined>("lastPlayedBy") || "unknown";
		await analyticsService.logActivity(player.guildId, currentUserId, "queue_end", {
			description: "Queue finished playing",
			timestamp: new Date().toISOString()
		}).catch((err) => {
			this.client.logger.error("Failed to log queue end event:", err);
		});
