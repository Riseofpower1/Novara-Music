	...
	public async run(interaction: any): Promise<void> {
		...
		const embed = this.client
			...
			.addFields(
				{
					name: "⏱️ Duration",
					value: isStream ? "🔴 LIVE STREAM" : this.client.utils.formatTime(duration),
					inline: true,
				},
				{
					name: "📊 Queue",
		...
		if (message) {
			const handleVolumeChange = async (change: number) => {
				...
				const volumeBar = "🔊 " + "█".repeat(Math.floor(vol / 10)) + "░".repeat(10 - Math.floor(vol / 10)) + ` ${vol}%`;
				await buttonReply(
					interaction,
					volumeBar,
				...
				await message.edit({
					embeds: [
						embed.setFooter({
							text: volumeBar + " | " + T(locale, "event.setupButton.volume_footer", {
								vol,
			...
			switch (interaction.customId) {
				...
				case "REWIND_BUT": {
					...
					await buttonReply(
						interaction,
						"⏪ -10s | " + T(locale, "event.setupButton.rewinded"),
						this.client.color.main,
					);
					await message.edit({
						embeds: [
				...
				case "PAUSE_BUT": {
					...
					await buttonReply(
						interaction,
						(player.paused ? "⏸️ " : "▶️ ") + T(locale, "event.setupButton.pause_resume", { name }),
						this.client.color.main,
					);
					await message.edit({
						embeds: [
							embed.setFooter({
								text: (player.paused ? "⏸️ " : "▶️ ") + T(locale, "event.setupButton.pause_resume_footer", {
									name,
									displayName: interaction.member.displayName,
								}),
								iconURL: interaction.member.displayAvatarURL({}),
				...
				case "FORWARD_BUT": {
					...
					await buttonReply(
						interaction,
						"⏩ +10s | " + T(locale, "event.setupButton.forwarded"),
						this.client.color.main,
					);
					await message.edit({
						embeds: [
				...
				case "SKIP_BUT": {
					if (player.queue.tracks.length === 0) {
						return await buttonReply(
							interaction,
							"⏭️ " + T(locale, "event.setupButton.no_music_to_skip"),
							this.client.color.main,
						);
					}
					...
					await buttonReply(
						interaction,
						"⏭️ " + T(locale, "event.setupButton.skipped"),
						this.client.color.main,
					);
					...
					const newEmbed = this.client
						...
						.setColor(this.client.color.main)
						.setDescription(
							`**[${newTrack?.title}](${newTrack?.uri})**\n\n🎤 **Artist:** ${newTrack?.title.split(" - ")[0] || "Unknown"}`,
						)
						.addFields(
							{
								name: "⏱️ Duration",
								value: newTrack?.isStream ? "🔴 LIVE" : this.client.utils.formatTime(newTrack?.duration || 0),
								inline: true,
							},
							{
								name: "📊 Queue",
				...
				case "LOOP_BUT": {
					...
					const loopEmoji = newLoop === "off" ? "🔁" : newLoop === "track" ? "🎵" : "📝";
					await buttonReply(
						interaction,
						loopEmoji + " " + T(locale, "event.setupButton.loop_set", {
							loop: newLoop,
						}),
						this.client.color.main,
					);
					await message.edit({
						embeds: [
							embed.setFooter({
								text: loopEmoji + " " + T(locale, "event.setupButton.loop_footer", {
									loop: newLoop,
									displayName: interaction.member.displayName,
								}),
								iconURL: interaction.member.displayAvatarURL({}),
				...
				case "STOP_BUT": {
					...
					await buttonReply(
						interaction,
						"⏹️ " + T(locale, "event.setupButton.stopped"),
						this.client.color.main,
					);
					await message.edit({
						embeds: [
							embed
								.setFooter({
									text: "⏹️ " + T(locale, "event.setupButton.stopped_footer", {
										displayName: interaction.member.displayName,
									}),
									iconURL: interaction.member.displayAvatarURL({}),
								})
								.setDescription("⏹️ " + T(locale, "event.setupButton.nothing_playing"))
